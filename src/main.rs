    ...
    task::spawn({
        async move {
            ...
            loop {
                ...
                println!("Collected task ids: {:?}", ids);
    ...
    for operator in operators {
        ...
        match sockets_map.get(&operator_id.to_vec()) {
            Some(mut socket) => {
                ...
                for ((response_name, response_value), (id, issued_task)) in response_json
                    .as_object()
                    .unwrap()
                    .iter()
                    .zip(ids_issued_tasks.clone())
                {
                    ...
                    let outputs_vector: Vec<(u16, Vec<u8>)> = match response_value
                    ...
                    match ids_issued_tasks.remove(&id) {
                        Some(task_issued) => {
                            ...
                            handle_bls_agg_response(
                                &Ok((bls_agg_response, response_digest_map.clone())),
                                secret_key.clone(),
                                l1_http_endpoint.clone(),
                                task_issuer,
                                quorum_nums.clone(),
